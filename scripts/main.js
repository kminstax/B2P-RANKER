...
function populateTableEntry(trainee) {
  ...
  <div class="table__entry ${eliminated}">
    ...
    <div class="table__entry-text">
      <span class="number"><strong>${trainee.number}</strong></span>
  ...
  <div class="ranking__entry ${eliminated}">
    <div class="ranking__entry-view">
      <div class="ranking__entry-icon">
        ...
        <div class="ranking__entry-icon-border ${trainee.rank.toLowerCase()}-rank-border" data-rankid="${currRank-1}"></div>
      </div>
      <div class="ranking__entry-icon-badge bg-${trainee.rank.toLowerCase()}">${currRank}</div>
      ${
        top8 ? '<div class="ranking__entry-icon-crown"></div>' : ''
      }
    </div>
    <div class="ranking__row-text">
      <div class="number"><strong>${trainee.number}</strong></div>
      <div class="name"><strong>${trainee.name_romanized}</strong></div>
      <div class="birthyear">${trainee.birthyear}</div>
    </div>
  </div>`;
  return rankingEntry;
}
...
function filterTrainees(event) {
  ...
  filteredTrainees = trainees.filter(function (trainee) {
    let initialMatch = includesIgnCase(trainee.name_romanized, filterText) || includesIgnCase (trainee.number, filterText) || includesIgnCase (trainee.birthyear, filterText) || includesIgnCase (trainee.nationality, filterText);
